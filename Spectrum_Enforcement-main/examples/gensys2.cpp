...
facility_ms *cpu;		// cpu facility 
facility_set *disk;		// disk drives
storage *mem;			// memory 
...
extern "C" void sim()				//1st process - job generator 
{
	...
	set_output_file(fp);
	max_events(1000);
	disk = new facility_set("disk", NDISKS);
	mem = new storage("memory", AMTMEM);
	systemBox = new box("system");
	enable_separate_stacks(2000);
	cpu = new facility_ms("cpu", NCPUS);
